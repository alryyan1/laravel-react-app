...
//        $this->middleware(['permission:add items']);
    ...
    public function balance()
    {
        ...
        $pdf->Cell($table_col_widht*4, 5, 'الاسم ', 'T,B', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht/1.5, 5, ' س.البيع', 'T,B', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht/1.5, 5, "وارد", 'T,B', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht/1.5, 5, "منصرف", 'T,B', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht/1.5, 5, "الرصيد  ", 'T,B', 1, 'C', fill: 0);
        $pdf->Ln();
        ...
        $pdf->head = function () use ($pdf,$table_col_widht,$arial,$page_width){
            ...
            $pdf->Cell($table_col_widht/1.5, 5, 'س.البيع', 'T,B', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht/1.5, 5, "وارد", 'T,B', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht/1.5, 5, "المنصرف  ", 'T,B', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht/1.5, 5, "الرصيد  ", 'T,B', 1, 'C', fill: 0);
            $pdf->Ln();
        ...
        foreach ($items as $item) {
            ...
            $pdf->Cell($table_col_widht/2, 5, $code_num, 0, 0, 'C',fill: $fill);
            $pdf->Cell($table_col_widht * 4, 5, $item->market_name, 0, 0, 'C', stretch: 1,fill: $fill);
            $pdf->Cell($table_col_widht/1.5, 5, $item->last_deposit_item->sell_price, 0, 0, 'C', stretch: 1,fill: $fill);
//            $pdf->Cell($table_col_widht/2, 5, $item->last_deposit_item->sell_price * $item->remaining, 0, 0, 'C', stretch: 1,fill: $fill);
            $total+= $item->sell_price * $item->remaining;
            $pdf->Cell($table_col_widht/1.5, 5, $total_deposit, 0, 0, 'C',fill: $fill);
            $pdf->Cell($table_col_widht/1.5, 5, $total_deduct, 0, 0, 'C',fill: $fill);
            $pdf->Cell($table_col_widht/1.5, 5, $item->remaining, 0, 1, 'C',fill: $fill);
            $pdf->Line(PDF_MARGIN_LEFT, $y, $page_width + PDF_MARGIN_RIGHT, $y);
            $index++;
            $code_num++;
...
//        $pdf->setLanguageArray($lg);
        ...
        $pdf->foot = function ($pdf) use ($page_width, $fontname,$settings) {
        ...
        $img = base64_decode(preg_replace('#^data:image/[^;]+;base64,#', '', $img_base64_encoded));
        if ($settings->is_logo ){
            $pdf->Image("@".$img, 155  , 0, 40, 40,align: 'C',);
            $pdf->Image("@".$img, 10  , 0, 40, 40,align: 'C',);
...
//        $arr = array('LR' => array('width' => 0.1, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)));
        ...
        $pdf->Cell($page_width, 5, $settings->hospital_name, 0, 1, 'C');
        $pdf->setFont($fontname, 'u', 17);

        $pdf->Cell($page_width, 5, $settings->address, 0, 1, 'C');
        $pdf->setFont($fontname, '', 10);
        $pdf->Ln();
        $pdf->Cell(30, 5, 'الرقم التعريفي', 0, 1, 'C');

        $pdf->Cell(30, 5, $settings->vatin , 0, 1, 'C');
        ...
        $pdf->Cell($table_col_widht*1.5, 5, $deduct->created_at->format('Y-m-d') , 0, 0, 'L');
        $pdf->Cell($table_col_widht, 5, "   ", 0, 1, 'C');
//        $pdf->Cell($table_col_widht, 5, $deduct->created_at->format('Y-m-d'), 0, 0, 'C');
        $pdf->Cell($table_col_widht, 5, ' ', 0, 0, 'C');
        $pdf->Cell($table_col_widht, 5, "", 0, 1, 'C');
        ...
        $pdf->Cell($table_col_widht, 5, 'Unit ', 'T,B', 0, 'C', fill: 0);
        ...
        foreach ($deduct->deductedItems as $item) {
            ...
            $pdf->Cell($table_col_widht, 5, $item->item?->type?->name, 0, 0, 'C');
...
//            dd($user->user_deposited);
    ...
    {
          $dsc  = new DoctorShiftController();
          $shifts =    $dsc->doctorVisitsByDate($request);
          $arr = [];

//          dd($shifts);
         $patients = $shifts->map(function (DoctorShift $doctorShift) use(&$arr){
            return  $doctorShift->visits->map(function ($visit) use(&$arr){
...
//        $pdf->setLanguageArray($lg);
        ...
        $page_width = $pdf->getPageWidth() - PDF_MARGIN_LEFT - PDF_MARGIN_RIGHT;
        $pdf->Cell(40, 5, 'From ', 1, 0, 'C', fill: 0);
        $pdf->Cell(40, 5, $request->get('first') ,1, 1, 'C');

        $pdf->Cell(40, 5, 'To ', 1, 0, 'C', fill: 0);
        $pdf->Cell(40, 5, $request->get('second'), 1, 1, 'C');
        $pdf->Cell($page_width, 5, 'Patients Report', 0, 1, 'C');
        $pdf->Ln();
        $table_col_widht = $page_width / 5;
        $pdf->setFillColor(200, 200, 200);


        $pdf->Cell($table_col_widht *1.5, 5, 'Name', 'TB', 0, 'C', fill: 1);
        $pdf->Cell($table_col_widht, 5, '  Date', 'TB', 0, 'C', fill: 1);
        $pdf->Cell($table_col_widht, 5, 'Doctor ', 'TB', 0, 'C', fill: 1);
        $pdf->Cell($table_col_widht/2, 5, 'User ', 'TB', 0, 'C', fill: 1);
        $pdf->Cell($table_col_widht, 5, 'Paid ', 'TB', 1, 'C', fill: 1);
...
//            dd($stdObj);
            $pdf->Cell($table_col_widht * 1.5, 5, $patient->patient->name, 'TB', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht, 5, $patient->patient->created_at, 'TB', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht, 5, $patient->patient?->doctor?->name, 'TB', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht/2, 5, $patient->patient->user->username, 'TB', 0, 'C', fill: 0);
            $pdf->Cell($table_col_widht, 5, $patient->total_services() + $patient->patient->lab_paid, 'TB', 1, 'C', fill: 0);
            $total += $patient->total_services() + $patient->patient->lab_paid;

        }
        $pdf->Cell($table_col_widht * 1.5, 5, '', 'TB', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht, 5, '', 'TB', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht, 5, '', 'TB', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht/2, 5, '', 'TB', 0, 'C', fill: 0);
        $pdf->Cell($table_col_widht, 5,$total , 'TB', 1, 'C', fill: 0);
...
//        return $patient;
    ...
    public function printPrescribedMedsReceipt(Request $request)
    {
        $patient = Doctorvisit::find($request->get('doctor_visit'));
//        return $patient;
        $count =  $patient->services->count();
...
//        $pdf->setLanguageArray($lg);
        ...
        $pdf->Cell(30,5,'Print Date & Time',0,0,'L',fill: 0);
        $pdf->Cell(30,5,$today  ,0,1,'C');
        $pdf->setMargins(5, 5, 5);
...
//        $pdf->Cell(60,5,$patient->created_at->format('Y/m/d H:i A'),0,1);
        ...
        $img = base64_decode(preg_replace('#^data:image/[^;]+;base64,#', '', $img_base64_encoded));
        if ($settings->is_logo ){
            $pdf->Image("@".$img, 10, 5, 20, 20,align: 'L');
...
//        $pdf->Ln();
        ...
        $pdf->SetFont($arial, '', 10, '', true);
        $pdf->Cell($page_width,5,'Medical Prescription   ',0,1,'C');
        $pdf->Ln();
...
//        $pdf->Cell($colWidth,5,'    الطبيب ',0,1,);
        ...
        $pdf->Cell($colWidth/2,5,' Age  :',0,0,);
        $pdf->Cell($colWidth*2  ,5,$patient->patient?->age_year,0,0,'C');
        $pdf->Cell($colWidth/2,5,' العمر ',0,1,'R');
        $y = $pdf->GetY();
...
//        $pdf->Ln(5);
        ...
        $pdf->Cell($page_width - 5,5,'R',0,1,align: 'L');


        $pdf->SetFont($arial, '', 8, '', true);
        $colWidth = $page_width/5;
        $pdf->Cell($colWidth * 1.5,5,'Name(الاسم)','TB',0,fill: 1);
        $pdf->Cell($colWidth,5,'Course(الكورس)','TB',0,fill: 1);
        $pdf->Cell($colWidth,5,'Route(المنفذ)','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'Days(الايام)','TB',0,fill: 1);
        $pdf->Cell($colWidth,5,'Note(ملحوظه)','TB',1,fill: 1);
        $total = 0;
        /** @var PrescribedDrug $prescription */
        foreach ($patient->patient->prescriptions as $prescription){
            $y = $pdf->GetY();
...
//        $pdf->MultiCell($page_width, 10, $patient->patient->prescription_notes, 0, 'L', 1);
        ...
        $pdf->Cell(25,5,'Signature' ,0,0,'L',fill: 0);
...
//        $pdf->setLanguageArray($lg);
    ...
    public function printLabAndClinicReceipt(Request $request)
    {
        $patient = Doctorvisit::find($request->get('doctor_visit'));
//        return $patient;
        $count =  $patient->services->count();
...
//        $pdf->setLanguageArray($lg);
        ...
        $pdf->SetFont($arial, '', 8, '', true);
        $colWidth = $page_width/2;
        $pdf->Cell($colWidth ,5,'Name','TB',0,fill: 1);
        $pdf->Cell($colWidth,5,'Price','TB',1,fill: 1);
        $pdf->Cell($colWidth ,5,'الاسم','TB',0,fill: 1);
        $pdf->Cell($colWidth,5,'السعر','TB',1,fill: 1);
        $total = 0;
        foreach ($patient->services as $requestedService){
            $pdf->Cell($colWidth ,5,$requestedService->service->name,'TB',0,stretch: 1);
            $pdf->Cell($colWidth,5,$requestedService->count * $requestedService->price,'TB',1);
            $total+= $requestedService->count * $requestedService->price;
        }
        foreach ($patient->patient->labrequests as $labrequest){
            $pdf->Cell($colWidth ,5,$labrequest->mainTest->main_test_name,'TB',0,stretch: 1);
            $pdf->Cell($colWidth,5,$labrequest->price,'TB',1);
        }
        ...
        $pdf->Cell($colWidth,5,'Grand Total',0,0,'C',fill: 0);
        $pdf->Cell($colWidth,5,$patient->total_services()  + $patient->patient->total_lab_value_unpaid()   ,0,0,'C');
        $pdf->Cell($colWidth,5,'المبلغ الاجمالي',0,1);
...
//    }
    public function printSale(Request $request)
    {
        ...
        $deduct = Deduct::find($request->get('deduct_id'));
        $count =  $deduct->deductedItems->count();
        $custom_layout = array(80, 140 + $count * 5);
        $settings= Setting::all()->first();
...
//        echo  $pdf->getPageWidth();
        ...
        $img = base64_decode(preg_replace('#^data:image/[^;]+;base64,#', '', $img_base64_encoded));
        if ($settings->is_logo ){
            $pdf->Image("@".$img, 22 , 0, 30, 30,align: 'C');
...
//        $pdf->Cell($page_width,5,'مسقط - عمان',0,1,'C');
        ...
        $pdf->Cell(30,5,$deduct->id,0,1,'');
        $pdf->Cell(10,5,'Date :',0,0);
        $pdf->Cell(30,5,$deduct->created_at->format('Y-m-d H:i A'),0,1,'');
        $pdf->Cell(10,5,'User :',0,0);
        $pdf->Cell(30,5,$deduct->user->username,0,1,'');
        $pdf->Cell(10,5,'To :',0,0);
        $pdf->Cell(30,5,$deduct->client->name ?? 'Default Client',0,1,'');
//        $pdf->Ln();
//        $pdf->Cell(15,5,'Date',0,0);
...
//        $pdf->Cell(25,5,'Requested Items',0,1,'L');
        ...
        $pdf->Cell($colWidth*2.7,5,'Item','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'Price','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'Qty','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'S.total','TB',1,fill: 1);
        $pdf->Cell($colWidth*2.7,5,'الصنف','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'السعر','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'كميه','TB',0,fill: 1);
        $pdf->Cell($colWidth/2,5,'اجمالي','TB',1,fill: 1);
        /** @var DeductedItem $deductedItem */
        foreach ($deduct->deductedItems as $deductedItem){
            $pdf->Cell($colWidth*2.7,5,$deductedItem->item->market_name,'TB',0,stretch: 1);
            $pdf->Cell($colWidth/2,5,$deductedItem->price,'TB',0);
            $pdf->Cell($colWidth/2,5,$deductedItem->box,'TB',0);
            $pdf->Cell($colWidth/2,5,$deductedItem->box * $deductedItem->price,'TB',1);
        }
//
...
//                $pdf->Ln();
        ...
        $pdf->Cell(20 ,5,'Total','TB',0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->total_price() ,'TB' ,0,'C',0);
        $pdf->Cell(20,5,'المجموع','TB',0,'C',fill: 0);
        $pdf->Cell(3 ,5,'',0,1,fill: 0);

        $pdf->Cell(3 ,5,'',0,0,fill: 0);
        $pdf->Cell(20 ,5,'Vat','TB',0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->calculateTax() ,'TB' ,0,'C',0);
        $pdf->Cell(20,5,'الضريبه','TB',0,'C',fill: 0);
        $pdf->Cell(3 ,5,'',0,1,fill: 0);


        $pdf->Cell(3 ,5,'',0,0,fill: 0);
        $pdf->Cell(20 ,5,'Total','TB',0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->calculateTax() + $deduct->total_price()  ,'TB' ,0,'C',0);
        $pdf->Cell(20,5,'المجموع','TB',0,'C',fill: 0);
        $pdf->Cell(3 ,5,'',0,1,fill: 0);

        $pdf->Cell(3 ,5,'',0,0,fill: 0);
        $pdf->Cell(20 ,5,'Payment',0,0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->paymentType->name ,0 ,0,'C',0);
        $pdf->Cell(20,5,'الدفع',0,0,'C',fill: 0);
        $pdf->Cell(3 ,5,'',0,1,fill: 0);


        $pdf->Cell(3 ,5,'',0,0,fill: 0);
        $pdf->Cell(20 ,5,'Discount','TB',0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->discount ,'TB' ,0,'C',0);
        $pdf->Cell(20,5,'الخصم','TB',0,'C',fill: 0);
        $pdf->Cell(3 ,5,'',0,1,fill: 0);


        $pdf->Cell(3 ,5,'',0,0,fill: 0);
        $pdf->Cell(20 ,5,'Paid','TB',0,'C',fill: 0);
        $pdf->Cell(20,5,$deduct->total_price() - $deduct->discount ,'TB' ,0,'C',0);
        ...
        $pdf->Cell($page_width,5,'Address:'.$settings->address,0,1,'C');
        //whatsapp icon
...
//        $pdf->setLanguageArray($lg);
        ...
        $pdf->setCreator(PDF_CREATOR);
        $pdf->setAuthor('alryyan mahjoob');
        $pdf->setTitle('Sickleave');
        $pdf->setSubject('Sickleave');
        $pdf->setMargins(5, 5, 5);
        $page_width = $pdf->getPageWidth() - 10;
        $arial = TCPDF_FONTS::addTTFfont(public_path('arial.ttf'));
        $pdf->AddPage();
        ...
        $pdf->SetFont($arial, '', 10, '', true);
        $pdf->Cell($page_width,5,'Certificate of Sick Leave for a Patient Attending in OPD/Emergency Dept.',0,1,'C');
        $cols = $page_width / 6;
        $pdf->Cell($cols,5,'Date of Issue :- ',0,0,'C');
        $pdf->Cell($cols,10,$patient->sickleave->created_at->format('Y-m-d'),0,1,'C');
        $y = $pdf->GetY();
