...
class DepositController extends Controller
{
    ...
    public function deleteZeroQuantity(Request $request, Deposit $deposit)
    {
        $count = DB::table('deposit_items')->where('deposit_id', $deposit->id)->where('quantity', '=', 0)->delete();
        return ['count' => $count, 'deposit' => $deposit->fresh()];
    }
    ...
    public function updateDepositItem(Request $request, DepositItem $depositItem)
    {
        ...
        return ['status' => $depositItem->update([$data['colName'] => $data['val']])];
    }
    ...
    public function uploadExcelToDeposit(Request $request, Deposit $deposit)
    {
        ...
        try {
            ...
            foreach ($data as $d) {
                ...
                $market_name = trim($d['market_name']);
                ...
                $item = false;
                $count = DB::table('items')->where('barcode', '=', $barcode)->count();
                if ($count > 0) {
                ...
                $count = DB::table('items')->where('market_name', '=', $market_name)->count();
                if ($count > 0) {
                    ...
                    $item = Item::create(['market_name' => $market_name, 'section_id' => null, 'require_amount' => 0, 'initial_balance' => 0, 'tests' => 0, 'unit' => 0, 'active1' => $active_1, 'active2' => $active_2, 'active3' => $active_3
                        , 'initial_price' => 0, 'approved_rp' => $sell, 'strips' => 1, 'pack_size' => $pack_size, 'barcode' => $barcode]);
                    $deposit_item = new DepositItem([
                        'item_id' => $item->id,
                        'cost' => $cost ?? 0,
                        'quantity' => 0,
            ...
            return ['status' => true, 'data' => $data, 'message' => $items_added . 'تم تعريف اصناف جديده '];
...
//        $deposit->items()->save($deposit_item);
    ...
    {
        $data = $request->all();;
        return ['status' => $depositItem->delete()];
    }
